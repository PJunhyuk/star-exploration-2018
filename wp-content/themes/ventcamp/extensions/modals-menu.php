...
function ventcamp_acf_custom_post_type() {

    $labels = array(
        'name'                => _x( 'Modals', 'Post Type General Name', 'vivaco' ),
        'singular_name'       => _x( 'Modals', 'Post Type Singular Name', 'vivaco' ),
        'menu_name'           => __( 'Vivaco Modals', 'vivaco' ),
        'parent_item_colon'   => __( 'Parent Modal', 'vivaco' ),
        'all_items'           => __( 'All Modals', 'vivaco' ),
        'view_item'           => __( 'View Modal', 'vivaco' ),
        'add_new_item'        => __( 'Add New Modal', 'vivaco' ),
        'add_new'             => __( 'Add New', 'vivaco' ),
        'edit_item'           => __( 'Edit Modal', 'vivaco' ),
        'update_item'         => __( 'Update Modal', 'vivaco' ),
        'search_items'        => __( 'Search Modals', 'vivaco' ),
        'not_found'           => __( 'Not Found', 'vivaco' ),
        'not_found_in_trash'  => __( 'Not found in Trash', 'vivaco' ),
    );

    $args = array(
        'label'               => __( 'Modals', 'vivaco' ),
        'description'         => __( 'Modals', 'vivaco' ),
        'labels'              => $labels,
...
add_action( 'init', 'ventcamp_acf_custom_post_type', 0 );
 ...
 * @return string
 */
function ventcamp_current_url($only_url = null, $port = null){
...
/**
 * Turn hex color values into RGB format
 *
...
function hex2rgb($hex) {
    ...
    // If color in short format, like so: #fff or #000
    if ( strlen( $hex ) == 3 ) {
...
function ventcamp_acf_modalbox_show(){
    ...
    $current_url = ventcamp_current_url(null, 'no');
    ...
    if(!empty($pops_to_display)){
        // Load modal boxes
        wp_enqueue_script( 'ventcamp-modal-box',    get_template_directory_uri() . "/js/ventcamp-modal-box.js", array('jquery'), false, true );

        foreach ($pops_to_display as $key => $popup) {

            echo ventcamp_acf_create_popup($popup);
        }
    }
}

// Add modal code to the footer
add_action('wp_footer', 'ventcamp_acf_modalbox_show', 1);
...
/**
 * Parse url and return only relative path,
 * without domain name and protocol
 *
...
function ventcamp_acf_create_popup($post_id){
